...
export class Console {
    ...
    dump_cmd(args) {
        ...
        while (sz > 0) {
            ...
            for (let i = 0; i < chunk_sz; ++i) {
                ...
                chars += byte >= 32 && byte < 127 ? Console.from_rk86_table[byte] : ".";
            }
            if (sz < WIDTH) {
    ...
    disasm_print(addr, nb_instr, current_addr) {
        ...
        while (nb_instr-- > 0) {
            ...
            this.term.writeln("%04X: %s%s %s %s".format(addr, current, bytes, chars, instr.instr));
            addr += instr.length;
        }
        return addr;
    }
    ...
    command(line) {
        ...
        if (argv.length > 0) {
            ...
            if (handler) handler[0].call(this, args);
    ...
    press(data) {
        const strokes = [
            [
                "\r",
                () => {
                    let cleared = this.line_buffer.trim();
                    this.line_buffer = "";
                    if (cleared.length === 0) {
                        if (this.history.length === 0) return;
                        cleared = this.history.at(-1);
                    }
                    if (this.history.at(-1) !== cleared) {
            ...
            [
                ...
                () => {
                    if (this.history_index === -1) return;
                    if (this.history_index < this.history.length - 1) this.history_index += 1;
                    const cmd = this.history[this.history_index];
                    this.term.write("\r\x1b[0K> " + cmd);
                    this.line_buffer = cmd;
                },
    ...
    init(machine) {
        ...
        terminal_panel.style.transform = "scale(1.0)";

        this.term.open(terminal_panel);
